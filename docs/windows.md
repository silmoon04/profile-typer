...
Download `ProfileTyper-0.5.2-windows-x64.zip` from the releases page. Extract the
...
The output is a folder in `dist/windows/0.5.2/ProfileTyper` and a ZIP in `dist`.
...
```powershell
.\dist\windows\0.5.2\ProfileTyper\ProfileTyper.exe --check C:\Temp\profile-typer-check
```
