...
- Named groups have a shared heading and border. Cards inherit the view or group
  tint; explicit statement and reason tones distinguish prose fields. Cards in a row
...
- Options wrap based on measured text and control widths, with padded selection
  backgrounds. A/B titles do not change card colors. Zero action counts are hidden.
